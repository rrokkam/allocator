...
Test(ye_memsuite, list_index, .init = setup, .fini = teardown) {
 cr_assert(seg_index(LIST_2_MIN) == find_list_index_from_size(LIST_2_MIN),
	 "List 2 min not placed in list 2");
 cr_assert(seg_index(LIST_2_MIN - 1) == 0,
	 "Number less than List 2 min not placed in list 1");
 cr_assert(seg_index(PAGE_SIZE << 1) == 3, "2048 not in list 4");
}
