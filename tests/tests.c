...
Test(ye_memsuite, realloc_smaller_block_splinter, .init = setup, .fini = teardown) {
    ...
    freelist *fl = &seglist[seg_index(PAGE_SIZE - 32)];

    // There should be only one free block in list 9
    cr_assert_not_null(fl->head, "No block in expected free list!");
    cr_assert(fl->head->alloc == 0, "Allocated bit is set!");
    cr_assert(fl->head->size << 4 == 992, "Free block size not what was expected!");
}
...
Test(ye_memsuite, realloc_smaller_block_free_block, .init = setup, .fini = teardown) {
    ...
    cr_assert_not_null(fl->head, "No block in expected free list!");
    cr_assert(fl->head->alloc == 0, "Allocated bit is set!");
    cr_assert(fl->head->size << 4 == PAGE_SIZE - 32, "Free block size not what was expected!");
}
...
Test(ye_memsuite, nextblock_prevblock, .init = setup, .fini = teardown) {
    void *x = ye_malloc(64);
    void *y = ye_malloc(96);
    ye_header *xhdr = (ye_header *)(x - HEADER_SIZE);
    ye_header *yhdr = (ye_header *)(y - HEADER_SIZE);
...
Test(ye_memsuite, malloc_overlap_pages, .init = setup, .fini = teardown) {
    ...
    ye_header *header = (ye_header *)((char *) y - 8);
    cr_assert(header->size << 4 == PAGE_SIZE + 2 * HEADER_SIZE, "Block size was not what was expected!");
    cr_assert(header->alloc == 1, "Allocated bit is not set!");

    cr_assert(ye_sbrk(0) - heap_min() == 2 * PAGE_SIZE, "Sbrk not called twice!");
}
